...
public class MountCommand(BotDbContext db, ILogger<MountCommand> logger) : IChatCommand
{
    ...
    public async Task ExecuteAsync(ITelegramBotClient botClient, CommandContext context, CancellationToken cancellationToken)
    {
        ...
        if (context.Argument is null)
        {
            ...
            await botClient.SendTextMessageAsync(context.Message.Chat.Id, CommandStrings.Mount_NoName, cancellationToken: cancellationToken);
            return;
        }
        ...
        {
            await botClient.SendTextMessageAsync(context.Message.Chat.Id, CommandStrings.Mount_NameTaken, cancellationToken: cancellationToken);
            return;
        }
        ...
        {
            await botClient.SendTextMessageAsync(context.Message.Chat.Id, CommandStrings.Mount_AlreadyExists, cancellationToken: cancellationToken);
            return;
        }
        ...
        await botClient.SendTextMessageAsync(context.Message.Chat.Id, CommandStrings.Mount_Success, cancellationToken: cancellationToken);
    }
}